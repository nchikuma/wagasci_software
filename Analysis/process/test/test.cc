...
void SpillCheck(string inputDir,string outputDir,string ingridDir,
    int runid, int acqid, int ing_start_runid, int ing_stop_runid);
bool SearchIngridRuns(vector<vector<int>> &ingrid_runids,string ingridDir,
    int ing_start_runid,int ing_stop_runid,
    int wg_start_time,int wg_stop_time);

int main(int argc, char** argv){
  int opt;
  int runid = -1;
  int acqid = -1;
  int ing_start_runid = -1;
  int ing_stop_runid  = -1;
  ...
  string ingridDir     = "/home/data/ingrid";
  delete con;
  ...
  while((opt = getopt(argc,argv, "f:d:r:s:o:i:j:h")) !=-1 ){
    switch(opt){
      case 'f':
        inputDirName = optarg;
      ...
      case 's':
        ...
        break;
      case 'i':
        ing_start_runid=atoi(optarg); 
        break;
      case 'j':
        ing_stop_runid=atoi(optarg); 
        break;
      case 'd':
        ingridDir=optarg;
        break;
      ...
      case 'h':
        ...
        cout <<"  -d <ingrid data dir> : INGRID data Directory" << endl;
        cout <<"  -i <ing_start_runid> : INGRID Run ID (start)" << endl;
        cout <<"  -j <ing_stop_runid>  : INGRID Run ID (stop)"  << endl;
        cout <<"  -o <outputDir>       : Output Directory (defualt: SPILL_DIRECTORY)" << endl;
        exit(0);
    }   
  }

  if(outputDirName==NULL||runid==-1||acqid==-1){
  ...
  SpillCheck(inputDirName,outputDirName,ingridDir,
      runid,acqid,ing_start_runid,ing_stop_runid);
    ...
    int runid, int acqid, int ing_start_runid, int ing_stop_runid)
{
  ...
  vector<vector<int>> ingrid_runids;
  bool isIngridFound = SearchIngridRuns(
      ingrid_runids,ingridDir,ing_start_runid,ing_stop_runid,starttime,stoptime);
...
{
  bool isFound = false;
  for(int irun=ing_start_runid;irun<=ing_stop_runid;irun++){
    for(int isrun=0;isrun<110;isrun++){
      string filename = Form("%s/ingrid_%08d_%04d_recon_B2.root",
          ingridDir.c_str(),irun,isrun);
      CheckExist *check = new CheckExist;
      if(check->RootFile(filename)){
        TFile *file = new TFile(filename.c_str(),"read");
        if(!file->IsZombie()){
          TTree *tree = (TTree*) file->Get("tree");
          int ing_start_time = (int)tree->GetMinimum("unixtime");
          int ing_stop_time  = (int)tree->GetMaximum("unixtime");
          if(ing_start_time<=wg_stop_time&&ing_stop_time>=wg_start_time){
            isFound = true;
            vector<int> tmp(2);
            tmp[0] = irun;
            tmp[1] = isrun;
            ingrid_runids.push_back(tmp);
          }
          else if(ing_start_time>wg_stop_time){
            return isFound;
          }
        }
        file->Close();
