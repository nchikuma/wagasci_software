...
int rbcp_com(char* ipAddr,
  ...
  for(itry=0;itry<3;itry++){
    sendto(sock, sndBuf, cmdPckLen, 0, 
        (struct sockaddr *)&sitcpAddr, sizeof(sitcpAddr));
    usleep(300000);
  }
  ...
  while(numReTrans<3){
    ...
    if(select(sock+1, &setSelect, NULL, NULL,&timeout)==0){
      ...
      if(FD_ISSET(sock,&setSelect)){
        ...
        if(RBCP_CMD_RD){
          memcpy(recvData,rcvdBuf+sizeof(struct rbcp_header),
              rcvdBytes-sizeof(struct rbcp_header));
        }
        ...
        if(dispMode==RBCP_DISP_MODE_DEBUG){
          ...
          for(i=0; i<rcvdBytes; i++){
            if(j==0) {
              printf("\t[%.3x]:%.2x %.2x",i, sendHeader->address,
