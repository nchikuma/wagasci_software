...
if  ssh wagasci-daq test -e /home/data/prototech/${run_name}; then
  ...
  echo "Before check, please run calibration_run_new.sh!"
  return
fi
...
for pe in `seq 2 3`; do
  ...
  echo "$WAGASCI_MAINDIR/bin/wgCalib -f ${CHECKXML_DIR} -m ${mode} -i ${CALIBIMG_DIR} -o ${CALIB_DIR} -p ${pe}" >> ${LOG}
